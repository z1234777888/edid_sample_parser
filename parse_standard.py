...
class BasicDisplayParameters:
    ...
    def parse_manager(edid_data: bytes):
        ...
        BasicDisplayParameters._video_input_definition(edid_data)

        print(f"輸入類型\t{BasicParams.input_type}")

        if BasicParams.input_type == "Digital":
            BasicDisplayParameters.ParseDigitalParams.parse_digital_params(edid_data)

            print(f"色彩位元\t{BasicParams.color_depth}")
            print(f"輸入介面\t{BasicParams.interface}")
        else:
            BasicDisplayParameters.ParseAnalogParams.parse_analog_params(edid_data)

            print(f"訊號基準\t{BasicParams.signal_level_standard}")
            print(f"單獨同步\t{BasicParams.separate_sync}")
            print(f"複合同步\t{BasicParams.composite_sync}")
            print(f"複合綠像\t{BasicParams.composite_sync_green_video}")
            print(f"垂直同步\t{BasicParams.serrations_v_sync}")
    ...
    def _video_input_definition(edid_data: bytes):
        ...
        BasicParams.input_type = "Digital" if video_input_type else "Analog"
    ...
    class ParseDigitalParams:
        ...
        def parse_digital_params(edid_data: bytes):
            ...
            # 獲取顏色深度描述
            BasicDisplayParameters.ParseDigitalParams._get_bit_depth(edid_data)
            # 獲取輸入介面描述
            BasicDisplayParameters.ParseDigitalParams._get_video_interface(edid_data)
        ...
        def _get_bit_depth(edid_data: bytes):
            ...
            BasicParams.color_depth = descriptions.get(bit_depth, "Undefined")
        ...
        def _get_video_interface(edid_data: bytes):
            ...
            BasicParams.interface = descriptions.get(interface, "Undefined")
    ...
    class ParseAnalogParams:
        ...
        def parse_analog_params(edid_data: bytes):
            """解析類比輸入參數"""
            BasicDisplayParameters.ParseAnalogParams._get_signal_level_standard(
                edid_data
            )
            BasicDisplayParameters.ParseAnalogParams._get_video_step(edid_data)
            BasicDisplayParameters.ParseAnalogParams._get_synchronization_types(
                edid_data
            )
            BasicDisplayParameters.ParseAnalogParams._get_serrations(edid_data)
        ...
        def _get_signal_level_standard(edid_data: bytes):
            ...
            BasicParams.signal_level_standard = descriptions.get(levels, "+0.7/−0.3 V")
